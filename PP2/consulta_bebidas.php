...
if (!empty($_POST['BotonConsultar'])) {
    ...
    $Mensaje = Validar_Seleccion_Consultas_Bebidas();
                    ...
                    <div class="card-body table-border-style">
                        ...
                        <form role="form" method="post">
	                        <div class="row">
	                            <div class="form-group col-md-4">
	                                ...
	                                <select class="form-control" name="bebida" id="bebida">
										...
										<option value="todos">Todas las bebidas</option>
										<?php
								        foreach ($ListadoBebidas as $bebida) {
								            echo '<option value="' . $bebida['IDBEBIDA'] . '">Id: ' . $bebida['IDBEBIDA'] . ' | ' . $bebida['BEBIDA'] . ' | ' . $bebida['MARCA'] . ' | ' . $bebida['VOLUMEN'] . '</option>';
								        }
										?>
									</select>
	                            </div>
	                        ...
	                        <table class="table">
	                            <thead>
	                                <tr>
	                                    <?php require_once 'vercolumnas.bebidasConsulta.inc.php'; ?>
	                                </tr>
	                            </thead>
	                            <tbody>
	                                <?php
	                                if (!empty($_POST['BotonConsultar'])) {
									    // Estoy en condiciones de poder validar los datos
									    $selectedBebida = $_POST['bebida'];
									    $contador = 1;
									    
									    if ($selectedBebida === "todos") {
									    ...
									    } else if (!empty($selectedBebida)) {
									        // Mostrar detalles del proveedor seleccionado
									        $detalleBebida = ObtenerBebidaPorId($MiConexion, $selectedBebida);
