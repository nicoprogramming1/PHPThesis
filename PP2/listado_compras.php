...
if (!empty($_POST['BotonModificar'])) {
    ...
    $Mensaje=Validar_Seleccion_Modificar_Compras();
    if (empty($Mensaje)) {
        // Obtener los IDs de la recepcion seleccionada
        $compraSeleccionada = $_POST['seleccionar'];

        // Redirigir a la página de modificación con los IDs seleccionados como parámetros
        header('Location: modificar_compra.php?id=' . implode(',', $compraSeleccionada));
        exit;
    }
}
                    	...
                    	<form role="form" method="post">
						    <div class="row">
						        <table class="table">
						            <thead>
						                <tr>
						                    <?php require_once 'vercolumnas.compras.inc.php'; ?>
						                </tr>
						            </thead>
						            <tbody>
						                <?php require_once 'listado.compras.inc.php'; ?>                                
						            </tbody>
						        </table>
						        <div class="col-md-12">
